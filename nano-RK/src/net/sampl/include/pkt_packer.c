...
uint8_t i;
      ...
      ds_pkt->epoch_time[0] = ds_pkt->buf[DS_EPOCH_TIME_0];
      ds_pkt->epoch_time[1] = ds_pkt->buf[DS_EPOCH_TIME_1];
      ds_pkt->epoch_time[2] = ds_pkt->buf[DS_EPOCH_TIME_2];
      ds_pkt->epoch_time[3] = ds_pkt->buf[DS_EPOCH_TIME_3];
...
ds_pkt->payload=(uint8_t *)&(ds_pkt->buf[ds_pkt->payload_start]);
...
{
	//ds_pkt->buf_len=DS_PAYLOAD_START+ds_pkt->payload_len+ds_pkt->mac_filter_num;
	ds_pkt->buf_len = ds_pkt->payload_start+ds_pkt->payload_len;
	...
	ds_pkt->buf[DS_EPOCH_TIME_3]=ds_pkt->epoch_time[3];
	ds_pkt->buf[DS_EPOCH_TIME_2]=ds_pkt->epoch_time[2];
	ds_pkt->buf[DS_EPOCH_TIME_1]=ds_pkt->epoch_time[1];
	ds_pkt->buf[DS_EPOCH_TIME_0]=ds_pkt->epoch_time[0];
...
uint8_t pack_gateway_packet( SAMPL_GATEWAY_PKT_T *gw_pkt)
{
	...
	gw_pkt->buf[GW_DST_SUBNET_0]=gw_pkt->dst_subnet_mac[0];
	gw_pkt->buf[GW_DST_SUBNET_1]=gw_pkt->dst_subnet_mac[1];
	gw_pkt->buf[GW_DST_SUBNET_2]=gw_pkt->dst_subnet_mac[2];
...
uint8_t unpack_gateway_packet( SAMPL_GATEWAY_PKT_T *gw_pkt)
{
      ...
      gw_pkt->dst_subnet_mac[0] = gw_pkt->buf[GW_DST_SUBNET_0];
      gw_pkt->dst_subnet_mac[1] = gw_pkt->buf[GW_DST_SUBNET_1];
      gw_pkt->dst_subnet_mac[2] = gw_pkt->buf[GW_DST_SUBNET_2];
...
uint8_t pack_peer_2_peer_packet( SAMPL_PEER_2_PEER_PKT_T *p2p_pkt)
{
	...
	p2p_pkt->buf[SUBNET_MAC_0]=p2p_pkt->src_subnet_mac[0];
	p2p_pkt->buf[SUBNET_MAC_1]=p2p_pkt->src_subnet_mac[1];
	p2p_pkt->buf[SUBNET_MAC_2]=p2p_pkt->src_subnet_mac[2];
	p2p_pkt->buf[P2P_DST_SUBNET_0]=p2p_pkt->dst_subnet_mac[0];
	p2p_pkt->buf[P2P_DST_SUBNET_1]=p2p_pkt->dst_subnet_mac[1];
	p2p_pkt->buf[P2P_DST_SUBNET_2]=p2p_pkt->dst_subnet_mac[2];
	...
	p2p_pkt->buf[P2P_HOP_CNT]=p2p_pkt->hop_cnt;
...
uint8_t unpack_peer_2_peer_packet( SAMPL_PEER_2_PEER_PKT_T *p2p_pkt)
{
      ...
      p2p_pkt->src_subnet_mac[0]= p2p_pkt->buf[SUBNET_MAC_0];
      p2p_pkt->src_subnet_mac[1]= p2p_pkt->buf[SUBNET_MAC_1];
      p2p_pkt->src_subnet_mac[2]= p2p_pkt->buf[SUBNET_MAC_2];
      p2p_pkt->dst_subnet_mac[0]= p2p_pkt->buf[P2P_DST_SUBNET_0];
      p2p_pkt->dst_subnet_mac[1]= p2p_pkt->buf[P2P_DST_SUBNET_1];
      p2p_pkt->dst_subnet_mac[2]= p2p_pkt->buf[P2P_DST_SUBNET_2];
      ...
      p2p_pkt->hop_cnt = p2p_pkt->buf[P2P_HOP_CNT];
